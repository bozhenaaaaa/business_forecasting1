...
Планова дата інтеграції: **Травень 2026 року** (кінець семестру).
...
### 15. Презентація
[[Посилання на презентацію в Canva](https://canva.link/np8921pirj4b6rf)] «Презентація готується до фінального захисту проєкту (кінець семестру).»
...
Файл з тестовими даними знаходиться у папці /data:

business_forecasting_dataset.xlsx

Датасет містить:
- SalesDataset (основна таблиця)
- Products
- Categories
- Stores
- Marketing

Дані використовуються для моделі прогнозування товарообігу.

## Вибір та запуск моделі

За результатами прогнозування найбільший товарообіг спостерігається в категорії Whiskey, далі Vodka. Категорії Wine та Champagne & Sparkling Wine мають нижчі, але стабільніші значення з помітними сезонними коливаннями. Отриманий файл прогнозу сформовано успішно та готово до подальшої інтеграції в аналіз.

Для побудови прогнозу використано датасет forecast_revenue_dataset.csv та notebook Model money.ipynb. У результаті виконання моделі сформовано файл money_three_phase_forecast.csv, який містить прогноз товарообігу по категоріях і датах. Для оцінки якості прогнозу застосовуються метрики RMSE та MAPE.

## Вибір та оцінка метрик

За результатами оцінки якості моделі прогнозу товарообігу мережі значення MAPE становить 19.3739%.
